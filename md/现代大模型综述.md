...
$$ \frac{\partial L*T}{\partial x_0} = \frac{\partial L_T}{\partial h_T} \cdot \frac{\partial h_T}{\partial h*{T-1}} \cdot \frac{\partial h*{T-1}}{\partial h*{T-2}} \cdots \frac{\partial h_1}{\partial h_0} \cdot \frac{\partial h_0}{\partial x_0} $$

其中关键项是连乘部分：

$$ \prod*{k=1}^{T} \frac{\partial h_k}{\partial h*{k-1}} = \prod\_{k=1}^{T} W_h^T \cdot \text{diag}(\sigma'(z_k)) $$
...
- **语义空间**：这个向量空间具有良好的几何性质。语义相近的词，在空间中的距离更近。
  - **经典例子**：$Vector(\text{King}) - Vector(\text{Man}) + Vector(\text{Woman}) \approx Vector(\text{Queen})$。
...
这种线性 Attention 可以被写成类似 RNN 的递归形式。定义记忆状态 $S_t = \sum_{i=1}^t \phi(k_i) v_i^T$：
$$ S*t = S*{t-1} + \phi(k_t) v_t^T $$
$$ y_t = \phi(q_t) S_t $$
...
在时刻 $t$，我们的目标是让记忆 $S$ 能够根据键 $K_t$ 正确检索到值 $V_t$。即希望：
$$ S \cdot K_t \approx V_t $$

为此，我们定义一个最小二乘损失函数（Loss Function）：
$$ \mathcal{L} = \frac{1}{2} \| S\_{t-1} K_t - V_t \|^2 $$

为了最小化这个损失，我们对 $S_{t-1}$ 进行一步梯度下降更新：
$$ \nabla*S \mathcal{L} = (S*{t-1} K*t - V_t) K_t^T $$
$$ S_t = S*{t-1} - \eta \cdot \nabla_S \mathcal{L} $$
...
1.  **预测 (Predict)**：$S_{t-1}K_t$ 是根据旧记忆对当前键的预测值。
2.  **误差 (Delta)**：$V_t - S_{t-1}K_t$ 是真实值与预测值的偏差。
3.  **修正 (Correct)**：我们只把这个**偏差**写入记忆。如果记忆已经能正确预测 $K_t$（即 $S_{t-1}K_t = V_t$），则 Delta 为 0，记忆保持不变。

这与卡尔曼滤波（Kalman Filter）中的“预测-校正”步骤有着异曲同工之妙。

如果展开公式，我们会发现它包含了一个显式的**减法项**：
$$ S*t = S*{t-1} \underbrace{- \beta*t (S*{t-1} K*t) \otimes K_t}*{\text{擦除旧信息}} + \beta_t V_t \otimes K_t $$
...
$$ S*t = A_t S*{t-1} + V*t K_t^T $$
这里没有 $\tanh$ 或 $\text{sigmoid}$ 包裹 $S*{t-1}$。正是这种**线性递推关系**，使得我们可以利用**结合律**进行并行训练（Parallel Scan / Chunkwise Computation）。
